...
PLOT_PATH = "img/DDPGv1"              # Guardar gráficas en esta ruta
...
class HybridEnv(gym.Env):
    def __init__(self, states, externals, reward_fn, reset_mask, dones, max_steps = MAX_ENV_STEPS):
    ...
    def step(self, action):
        ...
        input_sa = np.concatenate([action, external_vars, self.state])[None, :]
        next_state = ensemble_predict(tf.convert_to_tensor(input_sa, dtype=tf.float32))[0]

        # next_state = self.predict_fn(input_sa)[0]
...
env = HybridEnv(
    states = states,
    externals = externals,
    # predict_fn = ensemble_predict,  # 💡 aquí va la función
...
critic = tf.keras.models.load_model(CRITIC_INIT_PATH)
...
class NStepReplayBuffer:
    def __init__(self, n = N_STEP, gamma = GAMMA, max_size = BUFFER_SIZE):
        self.n = n
        self.gamma = gamma
        self.max_size = max_size
        self.ptr = 0
    ...
    def _store_transition(self, s, e, a, r, s2, e2, d, synthetic):
        if self.size < self.max_size:
            ...
            self.next_externals.append(e2)
            self.dones.append(d)
            self.is_synthetic.append(synthetic)
            self.size += 1
        else:
            self.states[self.ptr] = s
            self.externals[self.ptr] = e
            self.actions[self.ptr] = a
            self.rewards[self.ptr] = r
            self.next_states[self.ptr] = s2
            self.next_externals[self.ptr] = e2
    ...
    def _get_nstep_transition(self):
        R = 0.0
        for i, (_, _, _, r, _, _) in enumerate(self.nstep_buffer):
            R += (self.gamma ** i) * r

        state, external, action, _, _, _ = self.nstep_buffer[0]
        _, _, _, _, next_state, done = self.nstep_buffer[-1]

        return state, external, action, R, next_state, done

    def add(self, s, e, a, r, s2, e2, d, synthetic):
        self.buffer.append((s, e, a, r, s2, e2, d, synthetic))
        if len(self.buffer) >= self.n:
            R = 0
            for i in range(self.n):
                R += (self.gamma ** i) * self.buffer[i][3]
            s, e, a, _, _, _, _, synthetic_flag = self.buffer[0]
            sn, en, _, _, _, _, dn, _ = self.buffer[-1]
            self._store_transition(s, e, a, R, sn, en, dn, synthetic_flag)
            self.buffer.pop(0)
...
buffer = NStepReplayBuffer(n = N_STEP, gamma = GAMMA, max_size = BUFFER_SIZE)
...
for episode in range(NUM_EPISODES):
    ...
    for batch_episode in range(BATCH_EPISODE):  # 100 batches por episodio
        ...
        external_vars = externals[idx]
        state_input = tf.convert_to_tensor(np.concatenate([external_vars, state])[None, :], dtype=tf.float32)
        ...
        action = actor(state_input)[0].numpy().astype(np.float32)
        ...
        if buffer.size > BUFFER_SAMPLE:
            ...
            batch = buffer.sample(BUFFER_SAMPLE)
            tensors = [tf.convert_to_tensor(x, dtype=tf.float32) for x in batch]
    ...
    pred_state = ensemble_predict(tf.convert_to_tensor(input_sa, dtype=tf.float32))[0]
