...
OUTPUT_DIR = "models/AWR/Grid_ReLu"
...
ADV_BETAS = [1.0]            # Ya no aplican estos parámetros
ADV_CLIPS = [1.0]                  # Ya no aplican estos parámetros
...
def train_awr(beta, clip_weight, save_dir):
    ...
    # Ponderaciones usando ReLU + Normalización
    weights = np.maximum(0, advantages)  # ReLU: solo ventajas positivas

    if np.max(weights) > 0:
        weights = weights / np.max(weights)  # Normalización a [0, 1]
    else:
        weights = np.zeros_like(weights)  # En caso de que todas las ventajas sean <= 0
