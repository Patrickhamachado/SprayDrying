...
ACTOR_PATH =  "checkpoints/DDPGv1/torre_DDPG_actor_2499.keras"  #   'models/DDPGv1/torre_DDPG_actor.keras'  #    política entrenada
REPORT_PATH = 'reports/eval_torre_DDPG_v1.csv'
IMG_DIR = 'img/DDPGv1'
...
plot_histogram(real_s, gen_s, [f's{i + 1}' for i in range(16)], 'eval_hist_estados_v1_.png', 'Estado')
plot_histogram(real_a, gen_a, [f'a{i + 1}' for i in range(8)], 'eval_hist_acciones_v1.png', 'Acción')
...
plot_tendencias(real_s, gen_s, [f's{i + 1}' for i in range(16)], 'eval_tend_estados_v1.png')
plot_tendencias(real_a, gen_a, [f'a{i + 1}' for i in range(8)], 'eval_tend_acciones_v1.png')
...
plt.legend()
plt.grid(True)
plt.savefig(f"{IMG_DIR}/eval_recompensa_por_hora_v1.png")
plt.close()
