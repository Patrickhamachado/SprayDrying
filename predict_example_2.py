...
PREDICCION_PATH = 'data/predichos.csv'
...
if not data.empty:
    ...
    input_data = data[list_cols].values # Extract values for input columns
    # Obtener los valores reales para las columnas a predecir como una lista
    real_values = data[list_PREDICT].values[0].tolist()

    print("\nDatos de entrada para la predicción (muestra aleatoria):")
    # print(input_data[list_cols])
    print(input_data)
...
try:
    ...
    print(prediction)
    prediccion_pandas = pd.DataFrame(prediction)
    prediccion_pandas.to_csv(PREDICCION_PATH, index=True)
